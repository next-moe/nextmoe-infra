...
| 2026-09-18 | `v2-spec-nullable` (#266): adjudicated spec post-processing (anyOf-null wrapper from Go types, null in nullable enums), gate G18, a standard-validator test with an unfixed-document control, a portal builder branch, an oasdiff declaration | grok-4.6-xhigh | 911 s, 255 calls | Every adjudication as written; gates green; found an eleventh field (`CalendarList.meta`) the orchestrator's census missed and said why its own YAML control missed it too. Its G18 reuses the fixer's Go-type walk, so only the standard-validator test is independent; both mutations of the passes were killed. Acceptance changed `meta` to a value field (always present), rebuilt the 346-line oasdiff declaration and scheduled its deletion (substring entries would mask a later real removal), and added the 10 zh overlay strings it had no write path for |
| 2026-09-18 | `v2-problem-params` (#267): FieldError params in a sibling repo's exact shape, exact huma reason mapping over every `validation.Msg*`, an AST source gate with a temp-dir control, an Idempotency-Key in-flight state (SETNX, legacy-record replay), header + 409 declared on every POST | grok-4.6-xhigh | 2311 s, 430 calls | Implemented as written, `-race` clean, 11 named idempotency branches each asserting handler-run counts. Two out-of-list edits, both reported (a test fake, a G8 exception); acceptance replaced the exception by renaming the member to the sibling's `param_names`. It flagged a DB-only live test its change would break (empty title now TOO_SHORT) — acceptance fixed it and the integration job confirmed. The Redis store methods were verified against a throwaway Redis, which the sandbox cannot reach. Mutations of the claim and of the TOO_LONG mapping were killed (7 and 3 tests) |
| 2026-09-18 | `v2-works-page` (#268): page mode on two search-backed collections (exported embed, hand parser, depth cap with params, page-mode list finish), handler tests through a seam it had to add | grok-4.6-xhigh | 1481 s, 249 calls | No deviations. Its "anything wrong" list held every acceptance fix: the `total` doc now false in page mode, a filter doc not naming `page=`, two errors with empty `detail`, an unused parameter. No fake of the search service existed, so it added package-level function hooks; accepted. Missed at acceptance, caught by CI: its document test's control shelled out to `git show <base sha>`, which passed in the worktree and failed in CI's clone (and the sha was squashed away anyway); the control was dropped. Put "no test reads git history" in the next book. Blind spot: no live test binds OpenSearch, so page mode against a real index is verified on production after deploy |
