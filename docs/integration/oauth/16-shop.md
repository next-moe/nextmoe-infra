...
> **状态：已实现**（2026-09-23）。首个装扮类型是**头像框**。实现见 `apps/api/internal/platform/shop`，路由注册见 `cmd/oauth/main.go`，扣费走 [06 萌萌点账本](./06-moemoepoint.md)。
...
- **在哪买、到处戴**（Steam 模式）：物品可以是全站共享（`site_id` 为空）或站点独有（`site_id` 有值，只能在它自己的店面卖），但买到之后在所有站点都能显示。每个站点还可以单独选戴另一件（Discord 按服务器装饰的模式）。
- 物品只能**下架**，不能在发布后删除；已发布物品的素材**不可更换**（只能改名称和简介）——拥有者买的就是它现在的样子。
- 最低价由配置中心 `shop.min_price` 决定（默认 **100**，公开键，可在 `GET /settings` 读到）。

## 1. 头像框的渲染契约

主流做法（Discord 装饰、Steam 头像框）都是**画布比头像大、叠在头像外面**。

- **画布是头像的 1.2 倍**的正方形，头像圆居中，中心透明。素材上传时会校验：正方形、边长 192–1024 px、四角和中心透明。
- **两份素材**：
  - 静态 PNG（必需，≤ 512 KB）；
  - 带透明通道的动态 WebP（可选，≤ 2 MB，尺寸必须与静态图一致）。
- 素材按内容哈希存到图床的对象存储 `decorations/<sha256>.<ext>`，**原字节保存**（不经图床转码——图床会把一切转成静态 WebP），`Cache-Control: public, max-age=31536000, immutable`。URL 一旦下发就永远指向同一份内容。
- 渲染方（KunUI `KunAvatar` 已实现）必须：
...
这些端点花的是用户的萌萌点，所以**只接受账号中心的第一方会话**；带 `client_id` 的 OAuth access token 一律 `403 / 19012`。站点自己的店面（阶段 3）会另开授权方式。

| 端点 | 方法 | 用途 |
|---|---|---|
| `/shop/catalog` | GET | **公开**：在售商品（`Cache-Control: public, max-age=60`）|
...
- 整笔购买在**一个数据库事务**里：锁商品 → 检查上架状态、窗口、库存、每人限购、是否已永久拥有 → 写订单 → 账本扣费（`reason=purchase`、`source_app="shop"`，用户 → sink `shop`，站点独有物品进 `shop:site:<id>`，**服务端校验余额**）→ 发放拥有记录。任何一步失败，什么都不会发生。
...
| 端点 | 权限 | 说明 |
|---|---|---|
| `GET/POST /admin/shop/assets` | manage | 列出 / 上传素材（multipart `file`），返回 `key` 与 `url` |
