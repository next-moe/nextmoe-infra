...
| 16 | [shop.md](./16-shop.md) | 🛍️ **萌萌点商店与装扮（Tier A）**：物品 / 商品 / 订单 / 拥有 / 穿戴五个对象；首个装扮是**头像框**（1.2 倍画布、静态 PNG + 动态 WebP、原字节按内容哈希存图床）；`cosmetics` 字段出现在 `/users/batch`、`/auth/me`、`/users/:uuid`；购买一个事务内扣费 + 发放；最低价 `shop.min_price`（默认 100）；错误码 19xxx |
