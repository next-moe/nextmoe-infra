...
| 17 | [account-deletion.md](./17-account-deletion.md) | 🗑️ **注销账号（Tier A）**：账号中心 `/delete-account` 自助申请，邮箱验证码 + **7 天冷静期**，到期由 OAuth 执行（改写用户名/邮箱、删除第三方绑定、偏好、角色、会话等，不保留原邮箱）；下游用 client Basic auth 拉 **`GET /users/deleted`**（按注销时间的游标流）清理本地数据；`/users/batch` 多 `anonymized_at`；错误码 10020 / 10021 |
