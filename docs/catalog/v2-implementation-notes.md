...
28. **Withdrawn items were `404` on `GET /v2/news/{id}`, not `410 GONE`.** Obligation 3 (03 §2.4) asks for a tombstone on the detail face. `news/service/public.go` filtered `status = published AND dead_at IS NULL` for both list and detail, so list disappearance was correct and pending items never leaked — only the detail status code was wrong. Closed by 2.17.0; the declaration it forgot is deviation 119.
...
117. **Folders get a moderation face, because the cutover would otherwise drop one** (2026-09-07). `PATCH` and `DELETE /v2/moderation/folders/{id}` let a moderator rewrite or blank a folder's name and description, force it private, or delete it outright. This is not new capability: the forum has shipped `collection.edit_any` / `collection.delete_any` on its own collections all along, and moving the canonical store to the catalog without these two operations would have quietly retired a live moderator power over user-authored text that is now on a public face. Standing is `catalogPerm.Moderates` — the same union the rest of `/v2/moderation` uses. Two asymmetries against the owner's own operations are deliberate. A moderator **may blank the name**, which `UserFolderService.Create`/`Patch` refuse (`ErrFolderNameRequired`): the abuse being removed is usually the name itself, and choosing a replacement is the owner's to do — deviation 112 already has clients deriving a label for an unnamed folder. A moderator **may delete the default folder**, which the owner cannot: `is_default` is set by the folder's owner, so honouring it here would let anyone make a folder undeletable by marking it default. What a moderator may not touch is the folder's items — moderation acts on what a folder publishes, not on what somebody collected. `DELETE /v2/moderation/users/{uid}/folders` is the third operation and a different job: it is what an account deletion reaches for, removing every folder the account holds in one transaction and answering a receipt with the counts. The forum's purge has always deleted `galgame_collection` rows; without this the cutover would have left a purged person's collections sitting in the catalog with no face they could ever open again.

118. **A deleted folder used to leave its import provenance behind, and the next importer run would have written into the hole** (2026-09-07, found while planning the read cut, not yet realised in production — 0 orphans of 8,247 rows, the join verified against a positive control). `cmd/import-favorites` reads `catalog_user_folder_import` as "this source collection is already folder N" and does **not** check that N still exists: `importForumCollections` counts the collection as `FoldersReused` and writes its memberships straight at that id. Deleting a folder took its items but not its provenance row, so the first person to delete an imported collection would have armed the next re-run to write membership rows under a folder id nothing owns — invisible on every face, and still counted by the favorites popularity row of deviation 114. The row now goes with the folder on all three delete paths (`deleteFolderRows`, shared by the owner's delete, the moderator's, and the account purge), and `loadState` deletes anything already orphaned before answering, with a WARN naming the count. This was latent because the folder tables were five hours old when it was found; the read cut is what would have fired it.

119. **The news face lost the partner's own attribution text on the way to `/v2`, and 2.17.0 shipped a `410` the contract never declared** (2026-09-08). Three defects on one face, all of them things v1 or the spec already had.

    **`attribution` and `column_url` were dropped.** `news/dto.PublicNewsSource` carries four fields and its own comment says why they ride every item rather than the envelope: *"Attribution was the only condition Galgame 批评 attached, and click-through to the original was 月幕's; a consumer that renders a single item, or mixes sources into one stream, must still be able to honour both from the item alone."* `repr.NewsSource` shipped two of the four. The service loaded all four and the mapper threw two away, so the only condition either partner attached — 「可以，注明出处就行」, and the shape we promised for it was the reprint notice at the top of every item plus a link to the column index — stopped being expressible on the live face the day `/v1/news` became a tombstone. The notice is the partner's own sentence (`本条情报转载自 Galgame 批评的《Gal周报》专栏，点击标题可跳转至原文`), not a template a client composes, which is exactly why it has to be a field. Obligation 1 in `03 §2.4` now enumerates all four.

    **`lane` was a filter you could not read back.** Deviation 98 declared `lane=` on `listNews` and wired it through; `repr.NewsItem` has no `lane`. So a caller could narrow the feed by a fact the representation would not tell it — and the write face's `repr.NewsSubmission` has carried `lane` since D34, so the two faces disagreed about whether it exists. It is the source's own section label, not content: 月幕 serves `/open/topic/news` and `/open/topic/column` with an identical payload, and `lane` is the only thing that survives to tell them apart. Production is 4,585 `news` and 14 `column`.

    **The `410` was implemented and never declared.** 2.17.0 closed deviation 28 in `news/service/public.go` and `Catalog.NewsItem`, and left `getNewsItem` declaring `404, 503` with the description *"Withdrawn items are 404."* `contractWalk` cannot catch this: it asserts that a status it **observed** is in the declared set, and observing `410` needs a withdrawn row, which the credential-less walk has no database for. A mirror generating its client from the published spec therefore had no `410` branch while nine withdrawn rows sat in production — the precise failure obligation 3 exists to prevent, reintroduced one layer up. `TestNewsGoneIsDeclaredNotJustImplemented` asserts the declaration beside the behaviour, with the list face as the control.

    **Three fields were considered and deliberately left out**, and `03 §2.4` records why so the next wave does not read the omission as an oversight. `images[]` — 3,758 rows, all of them Galgame 批评's and none 月幕's, whose grant is `只取 preview message + banner image`; both partners' shape is click-through to the original, and publishing the full illustration set turns a pointer into a copy. That needs 雪阿宜's word, not a spec edit. `work_ids` — the table exists, the resolver was never written (the 12-news-feed charter says so), and `news_item_work` holds **0 rows**, so the field would be a permanent `[]` inviting downstream to depend on it. `publisher_uid` — the source-row-as-grant binding, a write-face fact with no read-side use; printing it on a credential-less face only exports the authorisation model.
...
- **MCP** — `cmd/mcp` loads `GET /v2/catalog/openapi.json` (or `KUN_MCP_OPENAPI_PATH`) and registers one read-only tool per GET on `/v2/catalog`, `/v2/news`, `/v2/problems`, `/v2/vocabularies`. Tool name = operationId. `nmk_` keys only. G10 compares tool params ⊇ HTTP query/path params. Since 2026-09-13 it **re-reads that document every `KUN_MCP_SPEC_REFRESH_SECONDS` (default 300)** and installs the tool set it describes, so a v2 deploy no longer needs an mcp restart to be visible. It was a startup snapshot before, and the documented remedy — restart the container after a v2 deploy — was not reachable from the deploy UI: a redeploy runs `docker compose up -d`, which leaves an unchanged image `Running` and reports success. A fetch that fails or a document that does not parse leaves the tool set in force unchanged.
