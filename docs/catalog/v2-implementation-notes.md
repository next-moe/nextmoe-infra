...
111. **Folder cursors are `cur_`-wrapped; the playtime lane they were cloned from ships raw, and still does** (2026-09-06). `repr.List` publishes `pattern: ^cur_[A-Za-z0-9._~-]+$` on every `next_cursor`, but `/v2/me/playtimes` emits its keyset key raw — `RFC3339Nano|work_id`, colons and pipe included — and the folder lanes, built "strictly on the playtime precedent", copied that faithfully: a bare decimal id on the list, the raw key on items. Nothing red-flagged it because no fixture overflows a page; only a `limit=1` crawl sees the emitted form (`TestFolderCursorsAreOpaque` now does exactly that). Both folder lanes now round-trip through `collect.EncodeCursor` / `DecodeCursor` before 2.12.0 ships, so the family is spec-honest from its first release. The playtime lane is left as it is **deliberately**: it is GA, deployed clients hold raw watermarks, and fixing it needs its own wave — emit wrapped while still parsing the raw legacy forms (the same shape as deviation 41's bare-time acceptance) — not a rider on this one.
