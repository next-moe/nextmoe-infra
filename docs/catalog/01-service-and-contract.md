...
- **v2 用户面(`/v2/me/*`、`/v2/moderation/*`)**:Bearer 用户访问令牌。这两族**默认按人判权**,不按「应用被同意了什么」判权;两组例外各有自己的 scope,因为它们动的都不是这个人独占的东西:
  ...
  - **收藏夹**——`/v2/me/folders`、`/v2/me/works`——要 `folder:read` / `folder:write`(写 scope 同时给读)。`GET /v2/me/folders/holdings`(批量成员关系)在前者之下;`GET /v2/me/works`(2.25.0 起,一次答多部作品的收藏夹、时长与游玩状态)答案里带收藏夹,所以吃同一条闸。两个前缀落在 `identity.go` 的 `folderScopedPrefixes`,闸与归类测试读同一张表。
