...
- **A constraint that is true but invisible from this file**: a version floor, an upstream bug, a required ordering. huma dropping the query parameters of an **unexported** embedded input struct is one — `CollectionInput` and `WorkSubInput` are exported for that reason alone, and a reader who does not know it will unexport them and ship routes whose `cursor`/`limit` are declared nowhere and bound to nothing.
...
- Unified docs portal: `docs-kungal.nextmoe.dev`; read the full ownership model (Tier A/B/C) from the sibling checkout at `../nextmoe-docs/docs/_meta/ownership.md` (relative to this repository root).

## Database schema changes → you must remind about migrations

**Whenever this change touches the database schema (a GORM model adding/changing a field or table, or raw SQL/constraints/indexes in `cmd/migrate*`), you must explicitly tell the user at the end of the task: whether a migration needs to run, which command to run, and against which database.** The deploy runs it for you now, but naming it is still how the user knows what a redeploy is about to execute against which database — and it is what surfaces the change that must run *before* the new code instead of alongside it.
...
- Why the reminder rule exists: in 2026-06 the `oauth_clients.moemoepoint_awarder` column was not migrated → the live code read a column that didn't exist (GORM `SELECT *` silently reads it as a zero value) and the entire site could not award moemoepoints for ~29h. The migrate jobs closed that gap afterwards; saying which migration a change carries is what keeps it closed.
