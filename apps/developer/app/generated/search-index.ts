...
export const searchIndex: SearchEntry[] = [
  ...
  },
  {
    ...
    "b": "searchCatalog /v2/catalog/search get Search catalog entities Cross-entity search. object= selects the family. Hits are search_result rows with target_object. Requires an application key or a user access token with catalog:read. cursor= pages the hits. ids= is not accepted. page= selects page mode (see the page parameter); every other collection is cursor-only. 跨实体搜索。object= 选择族。命中为带 target_object 的 search_result 行。需要应用密钥或带 catalog:read 的用户访问令牌。cursor= 翻页。不接受 ids=。page= 选择 page 模式（见 page 参数）；其余集合均为 cursor-only。 catalog:read cursor limit view include fields ids refs include_total facets sort nsfw page q object locale"
  },
  {
