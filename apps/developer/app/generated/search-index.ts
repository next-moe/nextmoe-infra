...
export const searchIndex: SearchEntry[] = [
  ...
  },
  {
    ...
    "b": "getCatalogTrait /v2/catalog/traits/{id} get Get one trait A character-trait vocabulary row. Without nsfw=true a sexual-family trait is 404 NOT_FOUND. include=aliases,description (and view=full) add those blocks. is_sexual reports the sexual-family flag. Requires an application key or a user access token with catalog:read. 一条角色特征词表行。未带 nsfw=true 时，性相关族特征返回 404 NOT_FOUND。include=aliases,description（以及 view=full）加入对应块。is_sexual 报告性相关族标志。需要应用密钥或带 catalog:read 的用户访问令牌。 catalog:read id nsfw view include fields"
  },
  {
