...
export const searchIndex: SearchEntry[] = [
  ...
  },
  {
    ...
    "b": "NextMoe API v2 的集合契约：list 信封、cur_ 游标、limit 上限、include_total、sort 与 facets，以及正确的翻页循环写法。 集合与分页 整个 API 的默认分页是 keyset 游标。所有集合共用同一个信封、同一套参数、同一条「翻到头了」的判据。唯一的例外是两个搜索集合另有页码模式。 list 信封 { \"object\": \"list\", \"items\": [ … ], \"next_cursor\": \"cur_…\", // 末页省略 \"total\": 12345, // 只在 include_total=true 时出现 \"facets\": { … }, // 只在 facets= 请求时出现 \"missing\": [\"…\"] // 只在 ids= / refs= 批量车道出现 } object 和 items 恒在。 items 是空数组时就是 [] ，永远不会是 null 。 游标 - next_cursor 是 不透明串 ，以 cur_ 开头。原样回传即可，不要解析、不要构造、不要基于它做算术。 - 末页直接不出现这个键。 没有 next_cursor: null ，也没有 has_more ——两个真值来源必然会不同步。 - 不要用 items.length === limit 判断还有没有下一页：满页末页必然说谎。 - 游标是 keyset 而不是 offset，所以深翻页不会越翻越慢，也不会因为中途有新行插入而重复或漏行。 正确的翻页循环长这样： let cursor do { const url = new URL('https://api.nextmoe.dev/v2/catalog/works') url.searchParams.set('limit', '100') if (cursor) url.searchParams.set('cursor', cursor) const page = await fetch(url, { headers: { Authorization: Bearer ${key} } }).then((r) = r.json()) for (const work of page.items) handle(work) cursor = page.next_cursor // 末页是 undefined，循环自然结束 } while (cursor) limit - 范围 1–100，默认 20。 - limit=101 是 400 LIMIT_TOO_LARGE ， 不会 被截断成 100。静默截断会让你以为自己拿到了全部。 total 默认不发 要精确总数就传 include_total=true 。它默认关闭是有代价考量的：带过滤条件的精确 COUNT 是对同一批数据的第二次全扫，最先在压力下超时，而且它和你刚拿到的那一页天然不一致（两次查询之间数据会变）。 做「共 N 页」的分页器请三思——游标集合的页码本来就没有稳定含义，做「加载更多」会更贴合。确实需要「跳到第 N 页」的浏览界面，用下面的页码模式。 页码模式 GET /v2/catalog/works 与 GET /v2/catalog/search 另收 page= 。这两个集合由搜索索引驱动，索引本来就按页取、总数精确，所以它们能兑现页码；其余集合没有 page= 。 { \"object\": \"list\", \"items\": [ … ], \"total\": 12345, // 页码模式下恒在，与 include_total 无关 \"total_relation\": \"eq\" // eq：total 精确；gte：total 是下界 } - page 从 1 开始，与 cursor 、 ids 、 refs 互斥。 - 响应 不带 next_cursor ，也不带 page_count ：用 total 与 limit 自己算。 - 深度上限 ： page × limit ≤ 10000 。越界是 400 INVALID_PARAMETER ， errors[0] 为 {\"parameter\": \"page\", \"reason\": \"OUT_OF_RANGE\", \"params\": {\"minimum\": 1, \"maximum\": <最大页 }} 。最后一页可达页码是 min(ceil(total / limit), floor(10000 / limit)) 。 - 在 /v2/catalog/works 上， page= 与 q= 、 facets= 、搜索排序一样会切到搜索车道，所以同样不能与 owner_uid= 、 site= 、 platform= 同用。 - 不要去解析游标里装的是什么，也不要自己拼游标来「模拟页码」：游标的内容随时可能改变，页码模式才是契约。 sort 每个集合声明自己的一套封闭 sort 键，未知值是 400 UNKNOWN_SORT 。排序一律带 tie-breaker——平局顺序不会由存储内部决定，所以重建索引不会打乱翻页。 以作品集合为例，它接受 id （默认）、 updated 、 relevance 、 released_desc 、 released_asc 、 popularity 。带 q= 做标题搜索时会切到搜索索引，此时只有 relevance 、 released_desc 、 released_asc 、 popularity 有意义。 facets facets= 请求分面计数，结果放在 facets 里，未知的分面名是 400 UNKNOWN_FACET 。作品集合支持 tag_id 、 company_id 、 olang 、 content_rating 、 medium 、 platform 。 GET /v2/catalog/works?facets=content_rating,medium&limit=1 批量车道 ids= / refs= 是 另一条车道 ：一次最多 100 个，没有分页， next_cursor 不会出现。请求了但不可见的 id 原样回在 missing[] 里，而不是让整个请求 404。详见 字段裁剪与批量读。 需要把整个目录同步下来时，不要靠翻 /v2/catalog/works 硬扫——用 /v2/catalog/changes 镜像信道，它按更新时间升序枚举整个人口，冷启动一次翻完，之后只要增量。",
    "h": [
      {
        "i": "envelope",
        "t": "list 信封"
  ...
  },
  {
    ...
    "b": "用 OAuth 用户访问令牌接入 NextMoe 的用户面：游玩时长、认领、编辑提案、封面投票、资讯投稿，以及审核队列。 接入用户数据 /v2/me 与 /v2/moderation 读写的是 某个用户自己的东西 。应用密钥在这两个前缀下一律无效——它证明不了「哪个用户」。 先拿到用户令牌 标准 OAuth 2.0 授权码 + PKCE，端点在 https://account.nextmoe.com/api/v1 。完整步骤见 鉴权与凭据。拿到 access_token 后照常放进 Authorization: Bearer ——和应用密钥同一个位置，但 不要同时带两个 。 用户面的限流按 用户 计数，不是按 IP。所以一定要带用户令牌调用；用别的方式代理会让你的全体用户挤进同一个桶。 六类用户数据 游玩时长 GET /v2/me/playtimes # 我的全部记录 GET /v2/me/playtimes/{work_id} # 单部作品 PUT /v2/me/playtimes/{work_id} # 覆盖写 POST /v2/me/playtimes # 批量写 DELETE /v2/me/playtimes/{work_id} 这一组 只要用户令牌 ，不需要任何额外 scope——任何已开通用户登录的应用都可以调用。批量写用 POST /v2/me/playtimes ，导入历史记录时别用 PUT 逐条打。 游玩状态 GET /v2/me/work-states # 我的全部状态 GET /v2/me/work-states/{work_id} # 单部作品 PUT /v2/me/work-states/{work_id} # 覆盖写 POST /v2/me/work-states # 批量写 DELETE /v2/me/work-states/{work_id} 状态是两个轴： state 是封闭词表 wish / doing / done / on_hold / dropped ，与 Bangumi 收藏类型一一对应； completion 表示通了多少（ one_route / main / all ），可以不填， wish 不允许携带它。 PUT 是整体替换——不带 completion 就等于清掉它。这一组和游玩时长一样 只要用户令牌 ，不需要任何额外 scope。 认领 GET /v2/me/claims POST /v2/me/claims # 投稿一个认领 GET /v2/me/claims/{id} PATCH /v2/me/claims/{id} # 需要 If-Match DELETE /v2/me/claims/{id} # 只能删草稿 认领是把目录里的一条记录和你自己站点的条目绑起来。提交时可以带 refs （外部锚）或 field_values （向导里用户填的字段图）。 带 refs 的提交如果命中了已存在的作品，会返回 409 而不是静默新建一条——这是防止你在目录里造出重复条目。先按 refs= 查一次，命中就走已有的那条。 编辑提案 GET /v2/me/proposals POST /v2/me/proposals # 提一个修改 GET /v2/me/proposals/{id} PATCH /v2/me/proposals/{id} # 修改或撤回，需要 If-Match POST /v2/me/proposals/{id}/amendments # 追加一条修正，需要 If-Match POST /v2/me/edit-images # 提案里要用的图先传这里 提案是「我想把这个字段改成那个值」。可改哪些字段由 GET /v2/catalog/schemas/{object} 的 fields 给出。列表带 include=patch （2.27.0 起）时每一行都有 patch 与 effective_patch ，与详情面一致，列表页不必逐条再读详情。提案与修订历史是 公开只读 的（ /v2/catalog/proposals 、 /v2/catalog/revisions ）——目录的每一次改动都留痕。 封面投票 GET /v2/me/cover-votes PUT /v2/me/cover-votes/{cover_id} DELETE /v2/me/cover-votes/{cover_id} 封面项的 id 在 GET /v2/catalog/works/{id}/covers 里——每个能被投票的东西都在读面上有地址，这是硬性设计约束的一个具体例子。 资讯投稿 GET /v2/me/news POST /v2/me/news # 投稿，总是落 pending GET /v2/me/news/{id} PATCH /v2/me/news/{id} # 编辑或撤回 投稿总是落在 pending ，由人工过审后才出现在公开的 /v2/news 。授权模型是「有源行即有资格」：你在资讯源表里有一行，就能替那个源投稿，没有单独的申请流程。 审核面 /v2/moderation/ 是给 有审核权的用户 用的，同样是用户令牌，能不能看见由那个用户在对应站点上的权限决定。 GET /v2/moderation/claims # 认领队列 POST /v2/moderation/claims/{id}/decisions # 通过 / 拒绝，需要 If-Match GET /v2/moderation/proposals # 提案队列 POST /v2/moderation/proposals/{id}/decisions # 需要 If-Match POST /v2/moderation/reverts # 回退到某个修订 GET /v2/moderation/snapshots/{object}/{id} # 当前编辑快照 每个租户只能看见和决定自己站点的东西，跨租户操作是 403 TENANT_MISMATCH 。 写操作的两个必修课 If-Match：不是可选的礼貌 PATCH /v2/me/claims/{id} 、 PATCH /v2/me/proposals/{id} 、 POST /v2/me/proposals/{id}/amendments 与两个 moderation 决策面都 要求 If-Match 。流程固定是「GET 拿 ETag → 带着它写」： const res = await fetch(url, { headers: auth }) const etag = res.headers.get('ETag') const current = await res.json() const write = await fetch(url, { method: 'PATCH', headers: { ...auth, 'If-Match': etag, 'Content-Type': 'application/json' }, body: JSON.stringify(patch) }) // 412 = 有人在你读之后改过：重新 GET，合并，再写一次 Idempotency-Key：网络超时不该写两条 所有 POST 都支持 Idempotency-Key 。同一身份、同一路径、同一个 key 的重复请求会重放首次结果（保留 24 小时）。key 请用 UUID 之类的一次性值，并且 同一个 key 只配同一个 body ——body 不同是 409 IDEMPOTENCY_KEY_REUSED ；首个请求还没处理完就重试是 409 IDEMPOTENCY_REQUEST_IN_PROGRESS ，写操作不会被执行两遍，稍等用同一个 key 重试即可。 - 拿用户令牌 — 授权码 + PKCE 的四步，以及刷新令牌的注意事项。 - 错误处理 — 412 / 428 / 409 分别意味着什么，该怎么恢复。",
    "h": [
      {
        "i": "token",
        "t": "先拿到用户令牌"
  ...
  },
  {
    ...
    "t": "sticker 表情包面接入",
    "s": "指南 · 下游站点面",
    "d": "接入 /v2/sticker 只读面：按 catalog 作品与角色身份索引的 Galgame 表情包素材。任意有效应用密钥即可调用，无需 scope；多语言标题、cursor 翻页、与 catalog 的连接方式、错误与缓存。",
    "b": "接入 /v2/sticker 只读面：按 catalog 作品与角色身份索引的 Galgame 表情包素材。任意有效应用密钥即可调用，无需 scope；多语言标题、cursor 翻页、与 catalog 的连接方式、错误与缓存。 sticker 表情包面接入 /v2/sticker 是 sticker.kungal.com 联邦进平台的只读面。 它不是第二份内容列表。 站上每一张表情都打了 infra catalog 的作品 id 与角色 id，所以这个面回答的是： 这个 catalog 身份有哪些表情素材？ 手里已经握着 catalog id 的调用方可以直接 join—— /v2/sticker/characters/{character_id}/stickers 与 /v2/sticker/works/{work_id}/packs 是主车道，其余端点都是围着它们的脚手架。 契约由表情包站自己的仓库拥有，本站只镜像它的 OpenAPI 原文；端点清单见端点参考 · sticker 表情包面。 数据边界 只暴露已发布的表情包。 草稿、隐藏、已删除的包，评论正文，审核状态，以及全部创作端点都不在这个面里，将来也不会有。 名字是多语言映射，不是字符串。 键为 zh-cn 、 zh-tw 、 ja-jp 、 en-us 、 und ，每一个都是可选的，任何一个都可能缺席； und 放的是 catalog 里没有语言标记的显示名。 { \"zh-cn\": \"夏日口袋\", \"ja-jp\": \"サマーポケッツ\", \"en-us\": \"Summer Pockets\" } 回退链由你自己定—— 这个面永远不替你挑一个 。 title 、 description 、 Work.name 、 Character.name 、 Tag.name 全部是这个形状。 拿密钥 Base URL 是 https://api.nextmoe.dev ，与 v2 同一个。凭据也是同一把：在控制台铸一把 nmk_live_ 应用密钥即可，流程见快速上手。 不需要任何 scope。 网关在请求到达表情包站之前跑 ForwardAuth，只查身份、速率与配额，不做授权——任意有效密钥都放行，永远不会 403。铸密钥时 不要去找 sticker:read ：这个 scope 字符串不存在，勾了会被拒。 curl \"https://api.nextmoe.dev/v2/sticker/packs?limit=3\" \\ -H \"Authorization: Bearer nmk_live_<YOUR_KEY \" X-API-Key: nmk_live_<YOUR_KEY 是等价写法。 只能从你自己的服务端调用。 网关对包括 OPTIONS 在内的每个方法都验密钥，而 CORS 预检不带认证头，所以浏览器直连必然 401——不是你的 CORS 配置写错了。详见鉴权与凭据。 九个端点 方法与路径 做什么 ---------------------------------------------------- -------------------------------- GET /v2/sticker/packs 列出已发布的表情包 GET /v2/sticker/packs/{pack_id} 单个包，含其中的表情、作品与角色 GET /v2/sticker/stickers/{sticker_id} 单张表情 GET /v2/sticker/characters 该站有素材的 catalog 角色索引 GET /v2/sticker/characters/{character_id} 单个 catalog 角色在该站的样子 GET /v2/sticker/characters/{character_id}/stickers 主车道 ：某个角色的全部表情 GET /v2/sticker/works 该站有素材的 catalog 作品索引 GET /v2/sticker/works/{work_id}/packs 主车道 ：关于某部作品的表情包 GET /v2/sticker/tags 标签，用得最多的在前 翻页与 catalog /v2 完全相同：列表信封是 {object, items, next_cursor} ，把 next_cursor 原样作为 cursor 传回取下一页， 末页直接不出现这个键 （不是 null ）； limit 取 1–100，默认 20，超过 100 是 400 LIMIT_TOO_LARGE ，不会被截断； total 默认不发，要就传 include_total=true 。不是这个面签发的 cursor 是 400 INVALID_CURSOR 。循环写法见集合与分页。 其余参数： sort 取 new （默认，按发布时间）或 hot （按下载数再按浏览数），两者都以 UUIDv7 的 id 收尾，所以是稳定的 tiebreaker； q 是不分大小写的子串匹配， 同时跨全部语言 ——一个日文查询能命中只有日文标题匹配的包，超过 100 字符会被截断。 nsfw 、 official 、 linked 声明为 字符串枚举 'true' / 'false' ，不是 JSON 布尔；照字面写 nsfw=true 即可。 nsfw 说的是 表情包自己的分级 ，不是它所属游戏的。从一部 r18 游戏里剪出来的日常反应表情包是 all_ages ，而它的 work.content_rating 仍然是 r18 。这里关联的 95 部游戏里有 89 部是 r18，按游戏分级过滤会几乎清空结果。 例一：某个角色的全部表情 主车道。手里有 catalog 角色 id 就直接问： curl \"https://api.nextmoe.dev/v2/sticker/characters/12345/stickers?limit=2\" \\ -H \"Authorization: Bearer nmk_live_<YOUR_KEY \" { \"object\": \"list\", \"items\": [ { \"object\": \"sticker\", \"id\": \"0193f2a1-…\", \"pack_id\": \"0193f29c-…\", \"position\": 1, \"image\": { \"hash\": \"3f7a…\", \"url\": \"https://…/full.webp\", \"thumb_url\": \"https://…/320.webp\", \"width\": 512, \"height\": 512 }, \"note\": \"…\", \"work\": { \"object\": \"work\", \"id\": \"61311\", \"name\": { \"zh-cn\": \"…\" } }, \"character\": { \"object\": \"character\", \"id\": \"12345\", \"name\": { \"ja-jp\": \"…\" } } } ], \"next_cursor\": \"cur_Mg\" } - 最新在前，只含已发布的包。 - 空页与 404 含义不同 ：角色没有素材时这里返回 空列表 ，绝不会仅因为这个 id 就 404。会 404 的是 GET /v2/sticker/characters/{character_id} ——那表示该站没有任何已发布的表情打了这个角色，它 不说明 catalog 认不认识这个 id 。 - image.hash 是 infra 图床服务的内容寻址，也是这里唯一稳定的标识符：同样的字节由生态内另一个站点存下来，hash 相同。表情包封面上没有这个字段，要稳定身份用 cover_sticker_id 。 - position 从 1 开始，在同一个包内唯一。 例二：关于某部作品的表情包 curl \"https://api.nextmoe.dev/v2/sticker/works/61311/packs?sort=hot&nsfw=true\" \\ -H \"Authorization: Bearer nmk_live_<YOUR_KEY \" { \"object\": \"list\", \"items\": [ { \"object\": \"pack\", \"id\": \"0193f29c-…\", \"title\": { \"zh-cn\": \"…\", \"ja-jp\": \"…\" }, \"description\": { \"zh-cn\": \"…\" }, \"official\": true, \"content_rating\": \"all_ages\", \"sticker_count\": 24, \"view_count\": 0, \"download_count\": 0, \"cover\": { \"url\": \"https://…/full.webp\", \"thumb_url\": \"https://…/320.webp\" }, \"cover_sticker_id\": \"0193f2a1-…\", \"work\": { \"object\": \"work\", \"id\": \"61311\", \"name\": { \"zh-cn\": \"…\" }, \"content_rating\": \"r18\" }, \"tags\": [ { \"object\": \"tag\", \"slug\": \"…\", \"name\": { \"zh-cn\": \"…\" }, \"pack_count\": 0 } ], \"author\": { \"object\": \"author\", \"id\": \"1\", \"name\": \"…\", \"avatar_url\": \"https://…\" }, \"created_at\": \"2026-08-29T02:51:07Z\", \"updated_at\": \"2026-08-29T02:51:07Z\", \"published_at\": \"2026-08-29T02:51:07Z\" } ] } 「关于」是宽口径：一个包只要 声明 了某部游戏， 或含有该游戏的表情 ，就算是关于它。站方预置的官方包一个游戏都不声明，却各自取材自几十部游戏——按窄口径读，它们什么都答不出来。 /v2/sticker/packs?work=61311 用的是同一条关系。 pack.work 只在作者声明过时出现；混合包不声明任何游戏，但每一张表情上仍带各自的 work 与 character 。 author.id 是 NextMoe 账号 id，与生态内每个站点用的是同一个。 例三：单个包，含全部表情 curl \"https://api.nextmoe.dev/v2/sticker/packs/0193f29c-…\" \\ -H \"Authorization: Bearer nmk_live_<YOUR_KEY \" 详情比列表多三个数组： stickers （按展示顺序）、 works （这些表情覆盖到的去重后游戏）、 characters （去重后角色）。 works 与 characters 就是这个包能 join 回 catalog 的全部身份，用它们做一次批量水合最省事。 pack_id 与 sticker_id 是 UUIDv7 字符串 ； work.id 、 character.id 、 author.id 是 十进制字符串 ，与 catalog /v2 的 id 同形，可以直接拿去拼 catalog 的路径。 和 catalog 拼起来 这个面给素材，catalog 给身份内容。两种方向： 从 catalog 往这边：我这部作品有没有表情素材 curl \"https://api.nextmoe.dev/v2/sticker/works/61311/packs\" \\ -H \"Authorization: Bearer nmk_live_<YOUR_KEY \" # 从这边往 catalog：拿角色 id 取 catalog 的权威资料 curl \"https://api.nextmoe.dev/v2/catalog/characters/12345\" \\ -H \"Authorization: Bearer nmk_live_<YOUR_KEY \" Work.id 拿去 /v2/catalog/works/{id} 解析， Character.id 拿去 /v2/catalog/characters/{id} 解析——同一把密钥就能读。 要 同步一份索引 而不是逐个查，用 GET /v2/sticker/characters ：它列出每个在已发布包中至少有一张表情的 catalog 角色 id，按数量排序。catalog 认识但该站没有素材的角色按设计不会出现在这里，所以这个索引就是「有素材的全集」。 GET /v2/sticker/works 同理，并在作品索引上额外下发 sticker_count 。 Work.content_rating 与 Character.image_url 都是 catalog 的值原样透传；catalog 标记角色立绘为限制级时 image_url 不下发。要严格的分级判定仍以 catalog 为准，见镜像到自己的库。 错误 这个面上的错误 全部 是 RFC 9457 application/problem+json ， code 取自平台那份封闭注册表。先按 HTTP status 分支，再看 code 。 网关写的（401、429） ——请求还没到表情包站就被拦下。 instance 是你请求的那条路径： HTTP/1.1 401 Unauthorized Content-Type: application/problem+json WWW-Authenticate: Bearer realm=\"nextmoe\", error=\"invalid_token\" { \"type\": \"https://developer.nextmoe.dev/problems/platform/invalid-credential\", \"title\": \"Invalid credential\", \"status\": 401, \"detail\": \"The application key is invalid, revoked or expired.\", \"instance\": \"/v2/sticker/packs?limit=20\", \"code\": \"INVALID_CREDENTIAL\", \"request_id\": \"req_01JBQ7X4M2K9P3W5T8ZVN6HRDC\", \"errors\": [] } 401 的 code 是 MISSING_CREDENTIAL （没带密钥）或 INVALID_CREDENTIAL （密钥无效、已吊销或已过期）；429 的是 RATE_LIMITED （每分钟速率）或 QUOTA_EXCEEDED （每日配额），两者都带 Retry-After （秒），并带 X-RateLimit- 与 X-Quota- 。 2026-09-18 之前，网关的 401 与 429 回的是平台旧信封 {\"code\":10001,\"message\":\"…\"} 。按 HTTP status 分支的代码不受这次统一影响；按 body 里 code 数值判断的代码需要改看 status 或新的字符串 code 。 表情包站写的（其余全部） ——RFC 9457 application/problem+json ，字段名与平台 /v2 的 problem 文档一致： HTTP/1.1 400 Bad Request Content-Type: application/problem+json { \"type\": \"https://developer.nextmoe.dev/problems/platform/limit-too-large\", \"title\": \"Limit too large\", \"status\": 400, \"detail\": \"limit must be at most 100; the value is not clamped\", \"instance\": \"/v2/sticker/packs?limit=500\", \"code\": \"LIMIT_TOO_LARGE\", \"request_id\": \"req_01JBQ7X4M2K9P3W5T8ZVN6HRDC\", \"errors\": [ { \"parameter\": \"limit\", \"reason\": \"OUT_OF_RANGE\", \"detail\": \"limit must be at most 100; the value is not clamped\" } ] } type URI 解析到本站的错误码注册表。 code 取自平台那份封闭注册表，原样照搬，因此一套客户端解码逻辑同时覆盖这个面与 catalog；这个面会出现的是 INVALID_PARAMETER 、 LIMIT_TOO_LARGE 、 INVALID_CURSOR 、 NOT_FOUND 、 METHOD_NOT_ALLOWED 、 INTERNAL_ERROR 、 SERVICE_UNAVAILABLE 。 detail 、 request_id （与 X-Request-ID 响应头同值）和 errors 恒在； 400 时 errors[0].parameter 指出要改的是哪个参数。 instance 是失败的那条请求的路径与查询串。 未知 slug 是个例外： tag= 传一个不存在的 slug 匹配不到任何东西 ，返回空列表，而不是报错。 一个分支写法： const res = await fetch(url, { headers: { Authorization: Bearer ${key} } }) if (res.status === 401) throw new Error('key missing or revoked') if (res.status === 429) { await sleep(Number(res.headers.get('Retry-After') ?? 60) 1000) return retry() } if (!res.ok) { const problem = await res.json() // application/problem+json throw new Error( ${problem.code}: ${problem.detail ?? problem.title} ) } 字段含义与分支顺序见错误处理。 限流 限流在网关按 密钥所属应用 计数，与 v2 共池：free 档 60 次/分、50,000 次/日。超限是 429 加 Retry-After ，并带 X-RateLimit- 与 X-Quota- 响应头。分档与退避写法见限流与配额。 同步索引时用满 limit=100 而不是默认 20，请求数直接降到五分之一。 缓存 每个 200 的 GET 都带 ETag 。把它原样放进下一次请求的 If-None-Match ，没变就是 304 ，不计入响应体传输： curl -i \"https://api.nextmoe.dev/v2/sticker/works/61311/packs\" \\ -H \"Authorization: Bearer nmk_live_<YOUR_KEY \" \\ -H 'If-None-Match: \"9f2a1c…\"' 这个面声明的是 Cache-Control: public, max-age=300, s-maxage=1800, stale-while-revalidate=3600 —— 可共享缓存 ，与 moyu 补丁面一致；错误文档是 no-store 。放一层自己的 CDN 或反向代理是安全的。完整契约见缓存与条件请求。 真正该缓存的是图片： image.url 与 image.thumb_url 指向图床 CDN，直接引用即可，既不经过这个面也不消耗 API 配额。 thumb_url 是 320px 变体，列表页用它。 image.hash 是内容寻址，可以直接当本地缓存键。 接下来 - 端点参考 · sticker 表情包面 —— 九个端点的全部参数、响应 schema 与可直接运行的 curl 示例。 - moyu 补丁面接入 —— 另一个下游站点面，同一把密钥、同一套约定。 - 鉴权与凭据 —— 为什么这个面不能从浏览器直接调。",
    "h": [
      {
        "i": "scope",
        "t": "数据边界"
  ...
  },
  {
    ...
    "b": "getCatalogTrait /v2/catalog/traits/{id} get Get one trait A character-trait vocabulary row. Without nsfw=true a sexual-family trait is 404 NOT_FOUND. include=aliases,description,intros (and view=full) add those blocks. include=character_count is the nightly index total that GET /v2/catalog/characters?trait_id=<this id>&page=1 answers under this request's nsfw; the engine failing is 503. It is an explicit ask: view=full does not add it. is_sexual reports the sexual-family flag. Requires an application key or a user access token with catalog:read. 一条角色特征词表行。未带 nsfw=true 时，性相关族特征返回 404 NOT_FOUND。include=aliases,description,intros（以及 view=full）加入对应块。include=character_count 是夜间索引上 GET /v2/catalog/characters?trait_id=<本 id>&page=1 在本请求 nsfw 下给出的总数；引擎失败为 503。这是显式请求：view=full 不会带上它。is_sexual 报告性相关族标志。需要应用密钥或带 catalog:read 的用户访问令牌。 catalog:read id nsfw view include fields"
  },
  {
  ...
  },
  {
    ...
    "t": "列出我的作品状态",
    "s": "端点 · 我的",
    "d": "GET /v2/me/work-states",
    "b": "listMyWorkStates /v2/me/work-states get List my work states The bearer user's per-work states. work_ids= is a batch read. Requires a user access token. Any app may call this; no scope is required. 持有者用户的逐作品状态。work_ids= 是批量读取。需要用户访问令牌。任何应用均可调用；不要求任何 scope。 cursor limit view include fields ids refs include_total facets sort nsfw work_ids"
  },
  {
    "r": "/docs/v2/batchMyWorkStates",
    "t": "批量写入作品状态",
    "s": "端点 · 我的",
  ...
  },
  {
    ...
    "t": "读取我对某部作品的状态",
    "s": "端点 · 我的",
    "d": "GET /v2/me/work-states/{work_id}",
    "b": "getMyWorkState /v2/me/work-states/{work_id} get Get my state on one work 404 when the user has never set one. Requires a user access token. Any app may call this; no scope is required. 用户从未设置过时为 404。需要用户访问令牌。任何应用均可调用；不要求任何 scope。 work_id"
  },
  {
    "r": "/docs/v2/putMyWorkState",
    "t": "替换我对某部作品的状态",
    "s": "端点 · 我的",
    "d": "PUT /v2/me/work-states/{work_id}",
    "b": "putMyWorkState /v2/me/work-states/{work_id} put Replace my state on one work Body carries state and optionally completion; an absent completion clears it. Naturally idempotent. Requires a user access token. Any app may call this; no scope is required. 请求体携带 state，可选 completion；缺省的 completion 即清除。天然幂等。需要用户访问令牌。任何应用均可调用；不要求任何 scope。 work_id"
  },
  {
    "r": "/docs/v2/deleteMyWorkState",
    "t": "删除我对某部作品的状态",
    "s": "端点 · 我的",
  ...
  },
  {
    ...
    "t": "我在各部作品上的收藏夹、游玩时长和游玩状态",
    "s": "端点 · 我的",
    "d": "GET /v2/me/works",
    "b": "listMyWorks /v2/me/works get My folders, playtime and play state, per work What the bearer has recorded about works: the folders holding each work, its playtime and its play state. With work_ids, up to 100 works in one request with no pagination: one item per distinct work id, in the order asked. A work the bearer has recorded nothing about still gets an item, with empty folder_ids and null playtime and work_state, and so does an id that names no work. Without work_ids, every work the bearer holds in a folder of their own, has a playtime on, or has a play state on, one item per work in ascending work id, paged with cursor and limit; include_total counts them. The values are the ones /v2/me/folders/holdings, /v2/me/playtimes and /v2/me/work-states answer; this face saves a client from asking all three. Cover votes are not included: they need catalog:edit, and /v2/me/cover-votes lists them. Requires a user access token with folder:read (folder:write also grants reads). 令牌持有人在作品上记下的内容：收着每部作品的收藏夹、游玩时长和游玩状态。带 work_ids 时，一次请求答最多 100 部作品，不分页：每个不同的作品 id 答一项，顺序与请求一致。持有人什么都没记的作品同样有一项，folder_ids 为空数组，playtime 与 work_state 为 null；不指向任何作品的 id 也这样回答。不带 work_ids 时，答出持有人放进自己收藏夹、记过游玩时长或设过游玩状态的每一部作品，每部一项，按作品 id 升序，用 cursor 与 limit 分页；include_total 给出总数。取值与 /v2/me/folders/holdings、/v2/me/playtimes、/v2/me/work-states 的回答一致，这个接口省去了分别调用三者。不含封面投票：它需要 catalog:edit，由 /v2/me/cover-votes 列出。需要带 folder:read 的用户访问令牌（folder:write 也可读）。 folder:read 或 folder:write cursor limit view include fields ids refs include_total facets sort nsfw work_ids"
  },
  {
  ...
  },
  {
    ...
    "b": "listPacks /v2/sticker/packs get List published sticker packs limit cursor include_total sort q nsfw tag work official linked"
  },
  {
  ...
  },
  {
    ...
    "b": "listWorks /v2/sticker/works get Catalog works this site has material for limit cursor include_total q"
  },
  {
  ...
  },
  {
    ...
    "b": "listTags /v2/sticker/tags get Tags, most used first limit cursor include_total"
  },
  {
  ...
  {
    "r": "/problems/platform/payload-too-large",
    "t": "请求体过大",
    "s": "错误码 · 平台通用",
    "d": "PAYLOAD_TOO_LARGE · HTTP 413",
    "b": "PAYLOAD_TOO_LARGE Payload too large The request body is larger than this operation accepts. Retrying the same body cannot succeed. 请求体超过了这个操作能接受的大小。原样重试不会成功。 platform"
  },
  {
