...
func sources() []model.CatalogSource {
	return []model.CatalogSource{
		...
		{ID: 19, Key: "nextmoe", TrustTier: 0, Note: "first-party measurements aggregated from our own users (playtime medians from catalog_user_playtime, gated on catalog_user_work_state done)"},
		...
		{ID: 21, Key: model.CensoredSourceKey, TrustTier: 0, Note: "first-party blurred stand-in derived from explicit official art (SFW cover face)"},
	}
}
