...
func translate(t *testing.T, baseURL string) (string, error) {
	t.Helper()
	zh, _, err := NewHTTPTranslator(baseURL, "t", "m", 64).Translate(context.Background(), "x", SourceJa, nil)
	return zh, err
}
...
func TestTheRetryReportsTheModelThatAnswered(t *testing.T) {
	...
	zh, model, err := NewHTTPTranslator(srv.URL, "t", "m", 64).Translate(context.Background(), "x", SourceJa, nil)
	require.NoError(t, err)
	assert.Equal(t, "译", zh)
