...
func resolveIDs(ctx context.Context, db *gorm.DB) (registryIDs, error) {
	...
	for key, dst := range map[string]*int16{
		"getchu": &r.getchu, "vndb": &r.vndb, "bangumi": &r.bangumi,
	} {
	...
	if r.getchu == 0 || r.vndb == 0 || r.bangumi == 0 || r.galgame == 0 {
		return r, fmt.Errorf("registry not seeded (getchu=%d vndb=%d bangumi=%d galgame=%d)", r.getchu, r.vndb, r.bangumi, r.galgame)
	}
	return r, nil
}

func loadSnapshot(ctx context.Context, db, gcDB *gorm.DB, ids registryIDs, now time.Time) (snapshot, error) {
	snap := snapshot{
		now: now, ids: ids,
		exactAnywhere: map[string]struct{}{},
		exactWork:     map[string]int64{},
...
func loadItems(ctx context.Context, gcDB *gorm.DB) ([]item, error) {
	...
	for i, r := range rows {
		out[i] = item{GetchuID: r.GetchuID, Title: r.Title, Brand: r.Brand, ReleaseDate: r.ReleaseDate}
	}
	return out, nil
}
