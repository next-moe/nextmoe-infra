    ...
    <div v-else class="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
      <KunCard v-for="item in items" :key="item.id" class="flex flex-col p-5">
        <div class="py-5">
          <ShopItemPreview
            :kind="item.kind"
            :static-url="item.preview?.static_url"
            :animated-url="item.preview?.animated_url"
          />
        </div>
        <div class="flex items-start justify-between gap-2">
          <div class="min-w-0">
            ...
            <p class="text-default-400 text-xs">
              #{{ item.id }} · {{ SHOP_KINDS[item.kind]?.label ?? item.kind }} ·
              {{ shopSites.nameOf(item.site_id) }}
            </p>
          </div>
        ...
        <div class="mt-auto flex flex-wrap justify-end gap-2 pt-4">
          ...
          <KunButton
            ...
            size="sm"
            :variant="
              a.action === 'publish' || a.action === 'relist' ? 'solid' : 'flat'
            "
            :color="
              a.action === 'retire' || a.action === 'reject'
                ? 'danger'
                : 'primary'
            "
