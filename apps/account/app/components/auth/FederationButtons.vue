<script setup lang="ts">
import {
  FEDERATION_PROVIDER_LABEL,
  FEDERATION_PROVIDER_MARK
} from '~/constants/federation'

const api = useApi()
const route = useRoute()

const providers = ref<{ name: string; label: string; mark?: string }[]>([])
const isLoading = ref(false)
const navigating = ref('')

const redirectUrl = computed(() => route.query.redirect as string | undefined)

const isSafeRedirect = (url: string): boolean => {
  if (url.startsWith('//')) return false
  if (url.startsWith('/')) return true
  try {
    return new URL(url).origin === window.location.origin
  } catch {
    return false
  }
}

const start = (name: string) => {
  isLoading.value = true
  navigating.value = name
  const r = redirectUrl.value
  const redirect = r && isSafeRedirect(r) ? r : ''
  window.location.href =
    resolveApiBase('oauth') +
    '/auth/federation/' +
    name +
    '/start' +
    (redirect ? '?redirect=' + encodeURIComponent(redirect) : '')
}

onMounted(async () => {
  const response = await api.get<FederationProvidersResponse>(
    '/auth/federation/providers'
  )
  if (response.code !== 0 || !response.data?.providers?.length) {
    return
  }
  providers.value = response.data.providers.flatMap((p) => {
    const label = FEDERATION_PROVIDER_LABEL[p.name]
    return label
      ? [{ name: p.name, label, mark: FEDERATION_PROVIDER_MARK[p.name] }]
      : []
  })
})
</script>

<template>
  <div v-if="providers.length" class="mt-8">
    <KunDivider class="mb-5">
      <span class="text-default-400 text-xs">或使用第三方账号</span>
    </KunDivider>

    <div class="space-y-2.5">
      <KunButton
        v-for="p in providers"
        :key="p.name"
        type="button"
        color="default"
        variant="bordered"
        size="lg"
        full-width
        class-name="justify-between gap-3 px-4"
        :disabled="isLoading"
        @click="start(p.name)"
      >
        <span class="flex min-w-0 items-center gap-3">
          <KunIcon
            v-if="navigating === p.name"
            name="lucide:loader-circle"
            class="text-default-500 size-5 shrink-0 animate-spin"
          />
          <img
            v-else-if="p.mark"
            :src="p.mark"
            alt=""
            width="20"
            height="20"
            class="size-5 shrink-0 rounded-md object-cover"
          >
          <svg
            v-else-if="p.name === 'google'"
            class="size-5 shrink-0"
            viewBox="0 0 24 24"
            aria-hidden="true"
          >
            <path
              fill="#4285F4"
              d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
            />
            <path
              fill="#34A853"
              d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
            />
            <path
              fill="#FBBC05"
              d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
            />
            <path
              fill="#EA4335"
              d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
            />
          </svg>
          <svg
            v-else-if="p.name === 'github'"
            class="size-5 shrink-0"
            viewBox="0 0 24 24"
            fill="currentColor"
            aria-hidden="true"
          >
            <path
              d="M12 2C6.477 2 2 6.484 2 12.017c0 4.425 2.865 8.18 6.839 9.504.5.092.682-.217.682-.483 0-.237-.008-.868-.013-1.703-2.782.605-3.369-1.343-3.369-1.343-.454-1.158-1.11-1.466-1.11-1.466-.908-.62.069-.608.069-.608 1.003.07 1.531 1.032 1.531 1.032.892 1.53 2.341 1.088 2.91.832.092-.647.35-1.088.636-1.338-2.22-.253-4.555-1.113-4.555-4.951 0-1.093.39-1.988 1.029-2.688-.103-.253-.446-1.272.098-2.65 0 0 .84-.27 2.75 1.026A9.564 9.564 0 0 1 12 6.844a9.24 9.24 0 0 1 2.504.337c1.909-1.296 2.747-1.027 2.747-1.027.546 1.379.202 2.398.1 2.651.64.7 1.028 1.595 1.028 2.688 0 3.848-2.339 4.695-4.566 4.943.359.309.678.92.678 1.855 0 1.338-.012 2.419-.012 2.747 0 .268.18.58.688.482A10.02 10.02 0 0 0 22 12.017C22 6.484 17.522 2 12 2Z"
            />
          </svg>

          <span class="min-w-0 truncate">使用 {{ p.label }} 继续</span>
        </span>

        <KunIcon
          name="lucide:arrow-right"
          class="text-default-300 size-4 shrink-0"
        />
      </KunButton>
    </div>
  </div>
</template>
